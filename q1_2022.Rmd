...
<script async src="https://www.googletagmanager.com/gtag/js?id=G-JMY3VD1QBD"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-JMY3VD1QBD');
</script>
...
fsc %>%  
  # filter(hub == "northeast") %>%
  mutate(strat_obj = str_to_upper(strat_obj)) %>%
  group_by(activity_red, strat_obj) %>% 
  summarise_at("beneficiaries", sum) %>% 
  pivot_wider(names_from = strat_obj, values_from = beneficiaries, values_fill = 0) %>% 
  left_join(
    fsc %>% 
      # filter(hub == "northeast") %>% 
      sum_ben(activity_red) %>% 
      rename(unique_beneficiaries = beneficiaries), 
    by = "activity_red"
  ) %>% 
  relocate(SO_3, .after = SO_2) %>% 
  rename(activity = activity_red) %>% 
  adorn_totals("row") %>% 
  kable(caption = "Breakdown of beneficiaries by activity in 2022/Q1", 
        format.args = list(big.mark = ",")) %>% 
  kable_classic_2("striped") %>% 
  footnote("The sum of beneficiary frequencies does not equal the numvber of unique beneficiaries reached", 
           general_title = "")
```
...
ggsave("cash_facet_partner.png", dpi = 300, height = 5, width = 8, units = "in")    
```
...
fsc %>% 
  ...
  labs(title = "Beneficiaries by township, Q1 2022",
