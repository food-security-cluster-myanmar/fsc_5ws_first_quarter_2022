---
title: "**Report on 5Ws: First Quarter 2022**"
...
state_ord <- fsc %>% 
  ...
  left_join(pin %>% 
              ...
              summarise(target = sum(fs_targeted, na.rm = TRUE))) %>% 
  mutate(pc_reached = round(beneficiaries / target * 100, digits = 2)) %>% 
  arrange(desc(beneficiaries)) %>% 
...
state_ord_2021 <- fsc_2021 %>% 
  ...
  filter(state %out% c("Bago (West)", "Nay Pyi Taw"))

fsc %>% 
  group_by(state, admin1_pcode) %>% 
  summarise(beneficiaries = sum(new_beneficiaries), .groups = "drop") %>% 
  ungroup() %>% 
  right_join(pin %>% group_by(state, admin1_pcode) %>% 
               summarise(target = sum(fs_targeted, na.rm = TRUE),
  ...
  select(State = state, Beneficiaries_2021 = beneficiaries_2021, `%_ben_2021` = pc_2021, 
         Beneficiaries_2022 = beneficiaries, `%_ben_2022` = pc_2022, `%_target_2022` = pc_reached) %>%
  ...
  column_spec(6, color = "white", background = spec_color(state_ord$pc_reached[1:16], option = "mako", end = .8, direction = -1)) %>% 
  kable_styling(latex_options = c("HOLD_position"), position = "center")
...
fsc %>%  
  ...
  kbl(caption = "Top 7 townships by beneficiaries reached (desc) in 2022/Q1", format.args = list(big.mark = ","), 
      booktabs = TRUE, linesep = "") %>% 
...
fsc %>% 
  ...
  kable(caption = "Breakdown of locations and beneficiaries by context", format.args = list(big.mark = ","), 
        booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position"), position = "center")
...
fsc %>% 
  ...
  summarise(beneficiaries = sum(beneficiaries)) %>% 
  ggplot(aes(x = fct_relevel(state, c("Kachin", "Mon", "Kayah", "Chin", "Kayin", "Shan (South)", 
                                      "Rakhine", "Shan (North")), 
...
The x-axis indicates the number of beneficiaries reached and the y-axis indicates the per-person value of each transfer. Each point is a distribution and the size of each point indicates the number of beneficiaries reached. The interactive version of this scatterplot is available in the [online edition](https://food-security-cluster-myanmar.github.io/fsc_5ws_first_quarter_2022/htmlwidget-e0905f35a4d4323f887f) of this report.
...
The plto below breask down the range of USD per person cash transfer values by state. Similar to the plot above, each point is a distribution. The interactive version of this scatterplot is available in the [online edition](https://food-security-cluster-myanmar.github.io/fsc_5ws_first_quarter_2022/#a-closer-look-at-food-distributions) of this report. 
...
```{r df-values-below, include=FALSE}
fsc %>% 
...
The plot below shows the number of beneficiaries and townships reached by each implementing partner. The interactive version of this scatterplot is available in the [online edition](https://food-security-cluster-myanmar.github.io/fsc_5ws_first_quarter_2022/#htmlwidget-95847ce66d4ecc260740) of this report.
...
fsc %>%
  ...
  labs(x = "Month", 
       ...
       title = "Monthly progress of top 20 implementing partners, 2021-2022/Q1", 
...
### 6.2 Township-level scatterplot of reach
...
The 12 townships along the extreme left side of the plot have beneficiaries but do not have targets. 230 townships with targets have not been reached. The interactive version of this scatterplot is available in the [online edition](https://food-security-cluster-myanmar.github.io/fsc_5ws_first_quarter_2022/township-level-scatterplot-of-reach) of this report. 
...
This interactive reference table is not available in this PDF edition. Please refer to the online version of this report [here](https://food-security-cluster-myanmar.github.io/fsc_5ws_first_quarter_2022/#htmlwidget-b1147b231fdb437722f7). 
